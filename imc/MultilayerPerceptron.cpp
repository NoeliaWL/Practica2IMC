...
void MultilayerPerceptron::weightAdjustment() {
	for(int h=1; h<nOfLayers; h++){
		for(int j=0; j<layers[h].nOfNeurons; j++){
			...
			if(online){
				layers[h].neurons[j].w[0] -= (eta * layers[h].neurons[j].deltaW[0]) - (mu * (eta * layers[h].neurons[j].lastDeltaW[0]));
			}
			else{
				layers[h].neurons[j].w[0] -= (eta * layers[h].neurons[j].deltaW[0])/nOfTrainingPatterns - (mu * (eta * layers[h].neurons[j].lastDeltaW[0]))/nOfTrainingPatterns;
			}
		}
	}
}
...
double MultilayerPerceptron::testClassification(Dataset* dataset) {
	...
	double maxIndexSalidaReal = 0, maxIndexSalidaObtenida = 0;
	...
	for(int i=0; i<dataset->nOfPatterns; i++){
		...
		for(int j=0; j<dataset->nOfOutputs; j++){
			...
			if(salidas[j] > maxIndexSalidaObtenida){
				maxIndexSalidaObtenida = salidas[j];
			}
		}
